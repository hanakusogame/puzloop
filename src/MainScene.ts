...
export class MainScene extends g.Scene {
	...
	public addScore: (n: number,time:number) => void;
	...
	constructor(param: g.SceneParameterObject) {
		param.assetIds = [
			...
			"map", "player", "ball", "combo", "clear", "effect",
			"config", "volume", "test", "glyph72", "number_w", "number_b", "number_p",
			"se_start", "se_timeup", "bgm", "se_clear", "se_move", "se_miss", "se_trush", "biri"];
		super(param);
		...
		this.loaded.add(() => {
			...
			//タイム
			uiBase.append(new g.Sprite({ scene: this, src: this.assets["time"], x: 5, y: 5 }));
			const labelTime = new g.Label({ scene: this, font: numFont, fontSize: 32, text: "70", x: 45, y: 8 });
			...
			this.addScore = (num: number,time:number = 0) => {
				...
				timeline.create().wait(time).call(() => {
